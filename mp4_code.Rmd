...
```{r}
hiit_1 <- plot_ly(hitters, x = ~yearID, color = I("black")) %>%
  add_markers(y = ~HR, text = ~paste('Player: ', nameFirst, "", nameLast), showlegend = FALSE) %>%
  add_lines(y = ~fitted(loess(HR ~ yearID)),
...
subplot(hiit_1, hiit_2, hiit_3 , hiit_4 , nrows = 2, margin = 0.06, heights = c(.55, .45), titleY = TRUE, titleX = TRUE)
```
