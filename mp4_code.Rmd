...
pitchers_not_inducted <- db %>%
  dbGetQuery("SELECT hof.playerID, hof.yearID, nameFirst,
              nameLast, min(hof.yearid) AS first_year, ballots,
              needed, votes, max(votes/ballots) as PCT, 
...
```{r warning = FALSE}
hittersss <- db %>% 
  dbGetQuery("SELECT hof.playerID, hof.yearID, nameFirst, hof.inducted,
              nameLast, sum(AB) as AB, sum(R) as R, sum(H) as H,
              sum(2B) as 2B, sum(3B) as 3B, sum(HR) as HR,
              sum(RBI) as RBI, sum(SB) as SB, 
              (sum(H) / sum(AB)) as AVG
...
ggplot(hittersss, aes(x = AVG, y = HR, colour = inducted)) +
  geom_point() +
  ggtitle("All Hitters")
```
...
pitchersss <- db %>% 
  dbGetQuery(
    "SELECT hof.playerID, hof.yearID, nameFirst, nameLast, hof.inducted,
    sum(G) as G, sum(pi.GS) as GS, sum(IPouts/3) as IP, 
    sum(CG) as CG, sum(SHO) as SHO, sum(SV) as SV, sum(W) as W,
    sum(L) as L, sum(H) as H, sum(BB) as BB, sum(ER) as ER,
    sum(SO) as SO, ((sum(ER) / (sum(IPouts)/3)) * 9) as ERA, 
...
ggplot(pitchersss, aes(x = ERA, y = W, color = inducted)) + 
  geom_point() +
